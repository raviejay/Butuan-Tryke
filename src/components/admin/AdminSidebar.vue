    ...
    <nav class="mt-6">
      <div class="px-4 space-y-2">
        ...
        >
          ...
          <button
          @click="$emit('section-change', 'fare-management')"
          :class="[
            'w-full text-left px-4 py-2 rounded-lg flex items-center transition-all duration-300',
            activeSection === 'fare-management'
              ? 'bg-orange-50 text-orange-600'
              : 'text-gray-700 hover:bg-gray-50',
            collapsed ? 'justify-center' : 'space-x-3',
          ]"
          :title="collapsed ? 'Fare Management' : ''"
        >
          <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              stroke-linecap="round"
          ...
          <span v-if="!collapsed" class="transition-opacity duration-300"> Fare Management </span>
        </button>
