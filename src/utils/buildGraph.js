    ...
    for (let i = 0; i < safeWaypoints.length; i++) {
      for (let j = i + 1; j < safeWaypoints.length; j++) {
        ...
        try {
         ...
         // Only add edges with ZERO violations
          if (!test.error && test.distance && test.violations === 0) {
            graph.edges.push({
              from: wpA.id,
              to: wpB.id,
              distance: test.distance,
              duration: test.duration,
              violations: 0, // Explicitly mark as safe
              cost: test.distance
            });
            console.log(`✅ ${wpA.id} -> ${wpB.id}: ${(test.distance/1000).toFixed(1)}km SAFE`);
          } else if (test.violations > 0) {
            console.log(`❌ ${wpA.id} -> ${wpB.id}: ${test.violations} violations - EXCLUDED`);
          }
        } catch (error) {
